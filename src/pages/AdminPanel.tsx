...
const AdminPanel = () => {
  ...
  const [editDialogGuideId, setEditDialogGuideId] = useState<string | null>(null);
  ...
  const generateQRCodeAndShareLink = async (guideId: string) => {
    try {
      ...
      const { error: updateError } = await supabase
        .from('audio_guides')
        .update({ qr_code_url: qrCodeDataUrl, share_url: shareUrl })
        .eq('id', guideId);
  ...
  const createGuide = async () => {
    ...
    const priceValue = formData.price?.trim() ? parseFloat(formData.price) * 100 : 0;
    ...
    try {
      ...
      if (isHidden) {
        toast.success('Hidden guide created! Only accessible via access link.');
      } else {
        toast.success('Published guide created! Discoverable on main page.');
      }
      ...
      if (error.message) errorMessage = `Failed to create guide: ${error.message}`;
    ...
    <div className="min-h-screen bg-background">
          ...
          <TabsContent value="content-management">
            <div className="space-y-6">
              ...
              <AdminGuideOrderManager 
                ...
                onEdit={(guideId) => setEditDialogGuideId(guideId)}
              />
            </div>
          </TabsContent>
      ...
      <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Create Audio Guide</DialogTitle>
          </DialogHeader>
              ...
              <CardContent className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input id="title" value={formData.title} onChange={(e) => handleInputChange('title', e.target.value)} placeholder="e.g., Goreme Open Air Museum" className="mt-1" />
                  </div>
                  <div>
                    ...
                    <Textarea id="description" value={formData.description} onChange={(e) => handleInputChange('description', e.target.value)} placeholder="Brief description of the guide..." maxLength={100} className="mt-1 min-h-[60px]" />
                    <Button type="button" variant="outline" size="sm" onClick={generateDescription} disabled={descriptionLoading || !formData.title || !formData.city || !formData.country || !formData.category} className="w-full mt-2">
                      {descriptionLoading ? <ButtonLoader text="Generating..." /> : 'Generate AI Description'}
                    </Button>
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <CountrySelector value={formData.country} onValueChange={(value) => handleInputChange('country', value)} placeholder="Select country" className="mt-1" />
                  </div>
                  <div>
                    <Label htmlFor="city">City</Label>
                    <Input id="city" value={formData.city} onChange={(e) => handleInputChange('city', e.target.value)} placeholder="e.g., Cappadocia" className="mt-1" />
                  </div>
                </div>

                <div>
                  <Label htmlFor="price">Price (USD)</Label>
                  <Input id="price" type="number" value={formData.price} onChange={(e) => handleInputChange('price', e.target.value)} placeholder="9.99" className="mt-1" />
                  <p className="text-xs text-muted-foreground mt-1">Set your guide price. Recommended: $3-15 for most guides</p>
                </div>

                <div>
                  <div className="flex items-center space-x-2">
                    <Switch id="featured-create" checked={formData.is_featured} onCheckedChange={(checked) => handleInputChange('is_featured', checked)} />
                    <Label htmlFor="featured-create" className="font-medium">Featured Guide</Label>
                  </div>
                  <p className="text-xs text-muted-foreground mt-1">Featured guides appear prominently on the homepage and get more visibility.</p>
                </div>

                <div className="space-y-2">
                  <Label>Guide Images</Label>
                  <ImageUploader onImagesUploaded={setUploadedImages} currentImages={uploadedImages} maxImages={5} />
                </div>
                    ...
                    <Card className="border-green-200 bg-green-50">
                      <CardHeader>
                        ...
                        <CardDescription className="text-green-700">
                          {isHidden ? 'Guide is hidden from main page - only accessible via this access link' : 'Guide is discoverable on main page with payment required - access link provides direct access'}
                        </CardDescription>
                      </CardHeader>
                      <CardContent className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          ...
                          <div className="space-y-4">
                            ...
                            <Button variant="outline" className="w-full" onClick={() => { setCreatedGuide(null); setQrCodeUrl(null); setShareUrl(null); }}>
                ...
                <Button onClick={createGuide} disabled={publishLoading} className="w-full" size="lg">
                  {publishLoading ? <ButtonLoader text="Creating Guide..." /> : <><Plus className="w-4 h-4 mr-2" />Create Guide (Admin)</>}
                </Button>
              </CardContent>
            </Card>
          </div>
      ...
      {/* Edit Guide Dialog */}
      <Dialog open={!!editDialogGuideId} onOpenChange={(open) => { if (!open) setEditDialogGuideId(null); }}>
        <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
          ...
          {editDialogGuideId && (
            <AdminGuideEditForm 
              onBack={() => setEditDialogGuideId(null)}
              guideId={editDialogGuideId}
              onClose={() => setEditDialogGuideId(null)}
            />
          )}
        </DialogContent>
      </Dialog>
