...
const AdminPreviewTab = () => {
  ...
  if (loading) return <AudioGuideLoader variant="page" message="Loading preview..." />;

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl sm:text-2xl font-bold">Preview & Test</h2>
      ...
      {previewUrl && (
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            <Button variant={iframeView === 'mobile' ? 'default' : 'outline'} size="sm" onClick={() => setIframeView('mobile')}>
              <Smartphone className="w-4 h-4 mr-1.5" /> Mobile
            </Button>
            <Button variant={iframeView === 'desktop' ? 'default' : 'outline'} size="sm" onClick={() => setIframeView('desktop')}>
          ...
          <div className="flex justify-center">
            <div className={`border rounded-2xl overflow-hidden shadow-lg bg-background transition-all duration-300 ${
              iframeView === 'mobile' ? 'w-[390px] h-[844px]' : 'w-full h-[800px]'
            }`}>
              <iframe src={previewUrl} className="w-full h-full border-0" title="Guide Preview" />
            </div>
          </div>
        </div>
      )}
...
export default AdminPreviewTab;
