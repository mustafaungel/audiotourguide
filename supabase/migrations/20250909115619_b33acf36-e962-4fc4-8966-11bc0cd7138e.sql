...
-- Remove any existing permissive storage policies
...
-- 6. Add security audit logging trigger for sensitive operations
...
CREATE TRIGGER audit_verification_requests
  AFTER SELECT OR INSERT OR UPDATE OR DELETE ON public.verification_requests
  FOR EACH ROW EXECUTE FUNCTION public.audit_sensitive_access();

CREATE TRIGGER audit_user_purchases
  AFTER SELECT OR INSERT OR UPDATE OR DELETE ON public.user_purchases
  FOR EACH ROW EXECUTE FUNCTION public.audit_sensitive_access();

-- 7. Create function to mask sensitive verification data
