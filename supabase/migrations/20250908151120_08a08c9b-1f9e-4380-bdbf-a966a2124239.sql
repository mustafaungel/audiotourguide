...
-- Step 1: Drop ALL existing policies (including service role policy)
...
-- Policy 1: Admins can manage all purchases (consolidated)
...
-- Step 4: Create secure function to get guest purchase info (NO EMAIL EXPOSURE)
...
AS $$
BEGIN
  -- Verify access code and guide match, return purchase info without email
  RETURN QUERY
  SELECT 
    up.id,
    up.guide_id,
