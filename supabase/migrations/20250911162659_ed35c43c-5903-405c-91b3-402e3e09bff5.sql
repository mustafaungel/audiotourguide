...
CREATE POLICY "Guide audio files are publicly accessible"
ON storage.objects
FOR SELECT
USING (bucket_id = 'guide-audio');

-- Policy for uploading guide audio files by authenticated users
CREATE POLICY "Authenticated users can upload guide audio"
ON storage.objects
FOR INSERT
WITH CHECK (
...
CREATE POLICY "Users can update their own guide audio"
ON storage.objects
FOR UPDATE
USING (
  bucket_id = 'guide-audio' 
...
CREATE POLICY "Users can delete their own guide audio"
ON storage.objects
FOR DELETE
USING (
  bucket_id = 'guide-audio' 
