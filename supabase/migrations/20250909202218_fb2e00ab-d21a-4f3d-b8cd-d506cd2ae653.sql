...
-- Phase 1: Drop unused creator-related tables
DROP TABLE IF EXISTS public.safe_creator_profiles CASCADE;
...
-- Phase 3: Remove creator-specific columns from profiles table
...
-- Phase 4: Simplify user_role enum to only include admin and user
...
-- Phase 5: Update audio_guides table to handle admin-only creation
-- Set all existing guides to be created by the first admin user
...
-- Phase 8: Remove unused creator-related triggers
DROP TRIGGER IF EXISTS audit_verification_document_access_trigger ON public.verification_requests;
DROP TRIGGER IF EXISTS audit_verification_document_changes_trigger ON public.verification_requests;
DROP TRIGGER IF EXISTS log_verification_document_access_trigger ON public.verification_requests;
DROP TRIGGER IF EXISTS log_verification_document_updates_trigger ON public.verification_requests;
DROP TRIGGER IF EXISTS update_verification_requests_updated_at_trigger ON public.verification_requests;

-- Phase 9: Clean up any remaining creator-related functions
...
-- Phase 10: Remove any creator-related tables we might have missed
...
-- Phase 11: Update existing functions to handle simplified system
-- Update handle_new_user function to work with simplified profiles
...
-- Phase 12: Security audit logging for cleanup
PERFORM public.log_security_event(
  ...
  null,
  jsonb_build_object(
    'cleanup_type', 'creator_system_removal',
    ...
    'functions_removed', 'creator_verification_functions',
    'policies_updated', 'contact_submissions_and_profiles',
