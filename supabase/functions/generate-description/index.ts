...
serve(async (req) => {
  ...
  try {
    const { title, country, city, category, duration } = await req.json();
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a professional travel content writer specializing in creating compelling descriptions for audio guides and cultural experiences.'
          },
          {
            role: 'user',
            content: prompt
